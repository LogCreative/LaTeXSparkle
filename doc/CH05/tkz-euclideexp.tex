...
\begin{tcbraster}[raster columns=3]
    ...
    \argbox{perpendicular/orthogonal 垂直}{
        ...
        过\textcolor{blue}{(经过点)}关于\textcolor{red}{(参照线段)}的垂直线。该条命令只有一个结果，通过\textbackslash \texttt{tkzGetPoint}得到。
    ...
    \argbox{parallel 平行}{
        ...
        过\textcolor{blue}{(经过点)}关于\textcolor{red}{(参照线段)}的平行线。该条命令只有一个结果，通过\textbackslash \texttt{tkzGetPoint}得到。
    }
    \argbox{bisector 角分线}{

    }
    \argbox{bisector out 外角分线}{
...
\begin{tcbraster}[raster columns=3]
    ...
    \auxcombox{$\backslash$tkzGetPoint 第三点}{
        {$\backslash$tkzDefTriangle}命令后紧跟 \texttt{$\backslash$tkzGetPoint(结果点)} 以得到三角形的第三点。
    }
